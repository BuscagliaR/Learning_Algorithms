                            ...
                            global.npc=TRUE, curve.seg.limit.combined=15, curve.seg.limit.single=5)
{
         ...
         && (seg.size<=curve.seg.limit.single)))
  {
    ...
    if(!do.bss && !global.npc)
    {
      ...
      fss.step.temp<-forward.ensemble(optimal.k.combined.segment.prob.array, optimal.k.combined.accuracy.mat, classes, seg.weight, thresh, do.par, cores=max.cores)
...
# ### ENSEMBLE ###
# 
# naive.ensembler<-function(predictions.matrix)
# {
...
# 
# 
# weighted.ensembler<-function(accuracy.vec, predictions.mat, ens.size)
# {
#   m=ens.size
...
# # use.cores=6
# 
# ensemble.sim<-function(preds.list, classes, n.x, folds.list)
# {
#   ### Begin Function
